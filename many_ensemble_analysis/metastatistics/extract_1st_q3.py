...
from paths import out_path_unmod, out_path_mod, meta_mod
...
for i in range(100):
    ...
    p0 = np.array([1, 2., 2.])
    nskip = 2
    p, p_std, f_std, return_data = fit(f, T_decaying[nskip:-3], deltaEs[nskip:-2], deltaEs_std[nskip:-2], p0
                                    , statistic=lambda means:np.array([deltaE_decay(means, beta/n_tau, i) for i in range(means.size - 1)])[nskip:-2]
                                    , data=decaying_regime
                                    #, p0_guesses=p0_guesses
                                    #, fit_kwargs={"method": 2}
                                    , jk_use_central_result=False
                                    )
    results_um.append([p[0], p_std[0]])
    ...
    p_m, p_std_m, f_std_m, return_data_m = fit(y, T_consider[:-1], me_means, me_stds, np.array([2])
            , statistic=lambda means: me(means, T_mod1[1] - T_mod1[0])
...
with open("results_1st_extraction.csv", "w") as fout:
